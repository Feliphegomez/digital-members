...
<?php
	...
	if(empty($current_user->membership_level))
		$confirmation_message = "<p>" . __('Your payment has been submitted. Your membership will be activated shortly.', 'digital-members-rfid' ) . "</p>";
	else
		$confirmation_message = "<p>" . sprintf(__('Thank you for your membership to %s. Your %s membership is now active.', 'digital-members-rfid' ), get_bloginfo("name"), $current_user->membership_level->name) . "</p>";
...
<?php if(!empty($dmrfid_invoice) && !empty($dmrfid_invoice->id)) { ?>
	...
	<?php
		...
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your membership account and a receipt for your initial membership invoice. A welcome email with a copy of your initial membership invoice has been sent to %s.', 'digital-members-rfid' ), $dmrfid_invoice->user->user_email) . "</p>";
	...
	?>
	<h3>
		<?php printf(__('Invoice #%s on %s', 'digital-members-rfid' ), $dmrfid_invoice->code, date_i18n(get_option('date_format'), $dmrfid_invoice->getTimestamp()));?>
	</h3>
	...
	<ul>
		...
		<li><strong><?php _e('Account', 'digital-members-rfid' );?>:</strong> <?php echo esc_html( $current_user->display_name );?> (<?php echo esc_html( $current_user->user_email );?>)</li>
		<li><strong><?php _e('Membership Level', 'digital-members-rfid' );?>:</strong> <?php echo esc_html( $current_user->membership_level->name);?></li>
		<?php if($current_user->membership_level->enddate) { ?>
			<li><strong><?php _e('Membership Expires', 'digital-members-rfid' );?>:</strong> <?php echo date_i18n(get_option('date_format'), $current_user->membership_level->enddate)?></li>
		<?php } ?>
		<?php if($dmrfid_invoice->getDiscountCode()) { ?>
			<li><strong><?php _e('Discount Code', 'digital-members-rfid' );?>:</strong> <?php echo esc_html( $dmrfid_invoice->discount_code->code );?></li>
		<?php } ?>
		...
		<?php if ( ! empty( $dmrfid_invoice->accountnumber ) || ! empty( $dmrfid_invoice->payment_type ) ) { ?>
			<div class="<?php echo dmrfid_get_element_class( 'dmrfid_invoice-payment-method' ); ?>">
				...
				<?php if($dmrfid_invoice->accountnumber) { ?>
					<p><?php echo esc_html( ucwords( $dmrfid_invoice->cardtype ) ); ?> <?php _e('ending in', 'digital-members-rfid' );?> <?php echo esc_html( last4($dmrfid_invoice->accountnumber ) );?>
					<br />
					<?php _e('Expiration', 'digital-members-rfid' );?>: <?php echo esc_html( $dmrfid_invoice->expirationmonth );?>/<?php echo esc_html( $dmrfid_invoice->expirationyear );?></p>
				<?php } else { ?>
...
<?php
	...
	{
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your membership account. A welcome email has been sent to %s.', 'digital-members-rfid' ), $current_user->user_email) . "</p>";
	...
	<ul>
		<li><strong><?php _e('Account', 'digital-members-rfid' );?>:</strong> <?php echo esc_html( $current_user->display_name );?> (<?php echo esc_html( $current_user->user_email );?>)</li>
		<li><strong><?php _e('Membership Level', 'digital-members-rfid' );?>:</strong> <?php if(!empty($current_user->membership_level)) echo esc_html( $current_user->membership_level->name ); else _e("Pending", 'digital-members-rfid' );?></li>
	</ul>
<?php
	}
?>
<p class="<?php echo dmrfid_get_element_class( 'dmrfid_actions_nav' ); ?>">
	<?php if ( ! empty( $current_user->membership_level ) ) { ?>
		<a href="<?php echo dmrfid_url( 'account' ); ?>"><?php _e( 'View Your Membership Account &rarr;', 'digital-members-rfid' ); ?></a>
	<?php } else { ?>
		<?php _e( 'If your account is not activated within a few minutes, please contact the site owner.', 'digital-members-rfid' ); ?>
	<?php } ?>
