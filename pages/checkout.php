...
<form id="dmrfid_form" class="<?php echo dmrfid_get_element_class( 'dmrfid_form' ); ?>" action="<?php if(!empty($_REQUEST['review'])) echo dmrfid_url("checkout", "?level=" . $dmrfid_level->id); ?>" method="post">
	...
	<?php if($dmrfid_review) { ?>
		<p><?php _e('Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'digital-members-rfid' );?></p>
	<?php } ?>
	...
	<?php
		...
		<div id="dmrfid_pricing_fields" class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout', 'dmrfid_pricing_fields' ); ?>">
			<h3>
				<span class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-h3-name' ); ?>"><?php _e('Membership Level', 'digital-members-rfid' );?></span>
				<?php if(count($dmrfid_levels) > 1) { ?><span class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-h3-msg' ); ?>"><a href="<?php echo dmrfid_url("levels"); ?>"><?php _e('change', 'digital-members-rfid' );?></a></span><?php } ?>
			</h3>
			<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-fields' ); ?>">
				<p>
					<?php printf(__('You have selected the <strong>%s</strong> membership level.', 'digital-members-rfid' ), $dmrfid_level->name);?>
				</p>
				...
				<div id="dmrfid_level_cost">
					<?php if($discount_code && dmrfid_checkDiscountCode($discount_code)) { ?>
						<?php printf(__('<p class="' . dmrfid_get_element_class( 'dmrfid_level_discount_applied' ) . '">The <strong>%s</strong> code has been applied to your order.</p>', 'digital-members-rfid' ), $discount_code);?>
					<?php } ?>
				...
				<?php if($dmrfid_show_discount_code) { ?>
					<?php if($discount_code && !$dmrfid_review) { ?>
						<p id="other_discount_code_p" class="<?php echo dmrfid_get_element_class( 'dmrfid_small', 'other_discount_code_p' ); ?>"><a id="other_discount_code_a" href="#discount_code"><?php _e('Click here to change your discount code.', 'digital-members-rfid' );?></a></p>
					<?php } elseif(!$dmrfid_review) { ?>
						<p id="other_discount_code_p" class="<?php echo dmrfid_get_element_class( 'dmrfid_small', 'other_discount_code_p' ); ?>"><?php _e('Do you have a discount code?', 'digital-members-rfid' );?> <a id="other_discount_code_a" href="#discount_code"><?php _e('Click here to enter your discount code', 'digital-members-rfid' );?></a>.</p>
					<?php } elseif($dmrfid_review && $discount_code) { ?>
						<p><strong><?php _e('Discount Code', 'digital-members-rfid' );?>:</strong> <?php echo $discount_code?></p>
					<?php } ?>
				<?php } ?>

				<?php if($dmrfid_show_discount_code) { ?>
				<div id="other_discount_code_tr" style="display: none;">
					<label for="other_discount_code"><?php _e('Discount Code', 'digital-members-rfid' );?></label>
					...
					<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php _e('Apply', 'digital-members-rfid' );?>" />
				</div>
				<?php } ?>
	...
	<div id="dmrfid_user_fields" class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout', 'dmrfid_user_fields' ); ?>">
		<hr />
		<h3>
			<span class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-h3-name' ); ?>"><?php _e('Account Information', 'digital-members-rfid' );?></span>
			<span class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-h3-msg' ); ?>"><?php _e('Already have an account?', 'digital-members-rfid' );?> <a href="<?php echo wp_login_url( apply_filters( 'dmrfid_checkout_login_redirect', dmrfid_url("checkout", "?level=" . $dmrfid_level->id . $discount_code_link) ) ); ?>"><?php _e('Log in here', 'digital-members-rfid' );?></a></span>
		</h3>
		<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-fields' ); ?>">
			<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-field dmrfid_checkout-field-username', 'dmrfid_checkout-field-username' ); ?>">
				<label for="username"><?php _e('Username', 'digital-members-rfid' );?></label>
			...
			<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-field dmrfid_checkout-field-password', 'dmrfid_checkout-field-password' ); ?>">
				<label for="password"><?php _e('Password', 'digital-members-rfid' );?></label>
			...
			<?php
				...
				if($dmrfid_checkout_confirm_password) { ?>
					<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-field dmrfid_checkout-field-password2', 'dmrfid_checkout-field-password2' ); ?>">
						<label for="password2"><?php _e('Confirm Password', 'digital-members-rfid' );?></label>
			...
			<?php
				...
				if($dmrfid_checkout_confirm_email) { ?>
					<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-field dmrfid_checkout-field-bconfirmemail', 'dmrfid_checkout-field-bconfirmemail' ); ?>">
						<label for="bconfirmemail"><?php _e('Confirm Email Address', 'digital-members-rfid' );?></label>
			...
			<div class="<?php echo dmrfid_get_element_class( 'dmrfid_hidden' ); ?>">
				<label for="fullname"><?php _e('Full Name', 'digital-members-rfid' );?></label>
				<input id="fullname" name="fullname" type="text" class="<?php echo dmrfid_get_element_class( 'input', 'fullname' ); ?>" size="30" value="" autocomplete="off"/> <strong><?php _e('LEAVE THIS BLANK', 'digital-members-rfid' );?></strong>
	...
	<?php } elseif($current_user->ID && !$dmrfid_review) { ?>
		<div id="dmrfid_account_loggedin" class="<?php echo dmrfid_get_element_class( 'dmrfid_message dmrfid_alert', 'dmrfid_account_loggedin' ); ?>">
			<?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">log out now</a>.', 'digital-members-rfid' ), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>
	...
	<div id="dmrfid_payment_method" class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout', 'dmrfid_payment_method' ); ?>" <?php if(!$dmrfid_requirebilling) { ?>style="display: none;"<?php } ?>>
		<hr />
		<h3>
			<span class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-h3-name' ); ?>"><?php _e('Choose your Payment Method', 'digital-members-rfid' ); ?></span>
		</h3>
		<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-fields' ); ?>">
			<span class="<?php echo dmrfid_get_element_class( 'gateway_paypal' ); ?>">
				...
				<a href="javascript:void(0);" class="<?php echo dmrfid_get_element_class( 'dmrfid_radio' ); ?>"><?php _e('Check Out with a Credit Card Here', 'digital-members-rfid' );?></a>
			</span>
			<span class="<?php echo dmrfid_get_element_class( 'gateway_paypalexpress' ); ?>">
				...
				<a href="javascript:void(0);" class="<?php echo dmrfid_get_element_class( 'dmrfid_radio' ); ?>"><?php _e('Check Out with PayPal', 'digital-members-rfid' );?></a>
			</span>
		</div> <!-- end dmrfid_checkout-fields -->
			...
			<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-fields' ); ?>">
				...
				<?php
					...
					<div class="<?php echo dmrfid_get_element_class( 'dmrfid_checkout-field dmrfid_payment-cvv', 'dmrfid_payment-cvv' ); ?>">
						<label for="CVV"><?php _e('Security Code (CVC)', 'digital-members-rfid' );?></label>
...
?>
				...
				?>
				<input type="checkbox" name="tos" value="1" id="tos" <?php checked( 1, $tos ); ?> /> <label class="<?php echo dmrfid_get_element_class( 'dmrfid_label-inline dmrfid_clickable', 'tos' ); ?>" for="tos"><?php printf(__('I agree to the %s', 'digital-members-rfid' ), $tospage->post_title);?></label>
			</div> <!-- end dmrfid_checkout-fields -->
	...
	<div class="<?php echo dmrfid_get_element_class( 'dmrfid_submit' ); ?>">
		...
		<?php if($dmrfid_review) { ?>
			...
			<span id="dmrfid_submit_span">
				...
				<input type="submit" id="dmrfid_btn-submit" class="<?php echo dmrfid_get_element_class( 'dmrfid_btn dmrfid_btn-submit-checkout', 'dmrfid_btn-submit-checkout' ); ?>" value="<?php _e('Complete Payment', 'digital-members-rfid' );?> &raquo;" />
			</span>
		...
		<span id="dmrfid_processing_message" style="visibility: hidden;">
			<?php
				$processing_message = apply_filters("dmrfid_processing_message", __("Processing...", 'digital-members-rfid' ));
