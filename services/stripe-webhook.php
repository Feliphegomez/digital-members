<?php
	...
	if(!empty($event_id))
	{
		try
		{
			...
			$dmrfid_stripe_event = Stripe_Event::retrieve($event_id);
		}
		catch(Exception $e)
		{
			$logstr .= "Could not find an event with ID #" . $event_id . ". " . $e->getMessage();
			// dmrfid_stripeWebhookExit();
			$dmrfid_stripe_event = $post_event;			//for testing you may want to assume that the passed in event is legit
		}
	}
	...
	//real event?
	if(!empty($dmrfid_stripe_event->id))
	{
		//check what kind of event it is
		if($dmrfid_stripe_event->type == "invoice.payment_succeeded")
		{
			if($dmrfid_stripe_event->data->object->amount_due > 0)
			{
				//do we have this order yet? (check status too)
				$order = getOrderFromInvoiceEvent($dmrfid_stripe_event);

				//no? create it
				if(empty($order->id))
				{				
					//last order for this subscription //getOldOrderFromInvoiceEvent($dmrfid_stripe_event);
					$old_order = new MemberOrder();
					$old_order->getLastMemberOrderBySubscriptionTransactionID($dmrfid_stripe_event->data->object->subscription);
					
					//lookup by customer id
					if(empty($old_order) || empty($old_order->id))
					{
						$old_order->getLastMemberOrderBySubscriptionTransactionID($dmrfid_stripe_event->data->object->customer);
					}
					...
					$invoice = $dmrfid_stripe_event->data->object;
					...
					$logstr .= "Created new order with ID #" . $morder->id . ". Event ID #" . $dmrfid_stripe_event->id . ".";
				...
				{
					$logstr .= "We've already processed this order with ID #" . $order->id . ". Event ID #" . $dmrfid_stripe_event->id . ".";
					dmrfid_stripeWebhookExit();
				}
			}
			else
			{
				$logstr .= "Ignoring an invoice for $0. Probably for a new subscription just created. Event ID #" . $dmrfid_stripe_event->id . ".";
				dmrfid_stripeWebhookExit();
			}
		}
		elseif($dmrfid_stripe_event->type == "invoice.payment_action_required") {
			// TODO: Test subs with SCA.
			$old_order = getOldOrderFromInvoiceEvent($dmrfid_stripe_event);
			$user_id = $old_order->user_id;
			$user = get_userdata($user_id);
			...
			$morder->invoice_url = $dmrfid_stripe_event->data->object->hosted_invoice_url;
			
			// Email the user and ask them to authenticate their payment.
			$dmrfidemail = new DmRFIDEmail();
			$dmrfidemail->sendPaymentActionRequiredEmail($user, $morder);

			// Email admin so they are aware.
			// TODO: Remove?
			$dmrfidemail = new DmRFIDEmail();
			$dmrfidemail->sendPaymentActionRequiredAdminEmail($user, $morder);
		...
		} elseif($dmrfid_stripe_event->type == "charge.failed")
		{
			...
			{
				$logstr .= "Could not find the related subscription for event with ID #" . $dmrfid_stripe_event->id . ".";
				if(!empty($dmrfid_stripe_event->data->object->customer))
					$logstr .= " Customer ID #" . $dmrfid_stripe_event->data->object->customer . ".";
				dmrfid_stripeWebhookExit();
			}
		}
		elseif($dmrfid_stripe_event->type == "customer.subscription.deleted")
		{
			...
			if(!empty($old_order)) {
				...
				if ( !empty( $dmrfid_stripe_event->data->object ) && 'subscription' == $dmrfid_stripe_event->data->object->object ) {
					
					$subscr = $dmrfid_stripe_event->data->object;
				...
				if(!empty($user->ID) && true === $cancel_membership ) {
					do_action( "dmrfid_stripe_subscription_deleted", $user->ID );
					
					if ( $old_order->status == "cancelled" ) {
						$logstr .= "We've already processed this cancellation. Probably originated from WP/DmRFID. (Order #{$old_order->id}, Subscription Transaction ID #{$old_order->subscription_transaction_id})\n";
					} else if ( ! dmrfid_hasMembershipLevel( $old_order->membership_id, $user->ID ) ) {
						...
						dmrfid_cancelMembershipLevel( $old_order->membership_id, $old_order->user_id, 'cancelled' );
					...
					$logstr .= "Subscription deleted for user ID #" . $user->ID . ". Event ID #" . $dmrfid_stripe_event->id . ".";
					dmrfid_stripeWebhookExit();
				} else {
					...
					$logstr .= "Could also be a subscription managed by a different app or plugin. Event ID # {$dmrfid_stripe_event->id}.";
					dmrfid_stripeWebhookExit();
				}
				
			} else {
				$logstr .= "Stripe tells us a subscription is deleted, but we could not find the order for that subscription. Could be a subscription managed by a different app or plugin. Event ID #" . $dmrfid_stripe_event->id . ".";
				dmrfid_stripeWebhookExit();
			}
		}
	}
	...
	function getUserFromInvoiceEvent($dmrfid_stripe_event)
	{
		//pause here to give DmRFID a chance to finish checkout
		sleep(DMRFID_STRIPE_WEBHOOK_DELAY);

		global $wpdb;

		$customer_id = $dmrfid_stripe_event->data->object->customer;

		//look up the order
		$user_id = $wpdb->get_var("SELECT user_id FROM $wpdb->dmrfid_membership_orders WHERE subscription_transaction_id = '" . esc_sql($customer_id) . "' LIMIT 1");
	...
	function getUserFromCustomerEvent($dmrfid_stripe_event, $status = false, $checkplan = true)
	{
		//pause here to give DmRFID a chance to finish checkout
		sleep(DMRFID_STRIPE_WEBHOOK_DELAY);
		...
		$customer_id = $dmrfid_stripe_event->data->object->customer;
		$subscription_id = $dmrfid_stripe_event->data->object->id;
		$plan_id = $dmrfid_stripe_event->data->object->plan->id;

		//look up the order
		$sqlQuery = "SELECT user_id FROM $wpdb->dmrfid_membership_orders WHERE (subscription_transaction_id = '" . esc_sql($customer_id) . "' OR subscription_transaction_id = '"  . esc_sql($subscription_id) . "') ";
		if($status)
	...
	// TODO Test this
    // TODO docblock
	function getOldOrderFromInvoiceEvent($dmrfid_stripe_event)
	{
		//pause here to give DmRFID a chance to finish checkout
		sleep(DMRFID_STRIPE_WEBHOOK_DELAY);
		...
		if ( ! empty( $dmrfid_stripe_event->data->object->subscription ) ) {
            $subscription_id = $dmrfid_stripe_event->data->object->subscription;
        } else {
            $subscription_id = $dmrfid_stripe_event->data->object->id;
        }
		...
		$old_order_id = $wpdb->get_var("SELECT id FROM $wpdb->dmrfid_membership_orders WHERE (subscription_transaction_id = '" . $customer_id . "' OR subscription_transaction_id = '"  . esc_sql($subscription_id) . "') AND gateway = 'stripe' ORDER BY timestamp DESC LIMIT 1");

		// since v1.8, DmRFID may store the Stripe subscription_id (sub_XXXX) instead of the Stripe customer_id (cus_XXXX)
		...
		if(empty($old_order_id))
		{
			...
			$invoice_id = $dmrfid_stripe_event->data->object->invoice;
			...
			if (isset( $invoice->subscription )) {
				...
				$old_order_id    = $wpdb->get_var( "SELECT id FROM $wpdb->dmrfid_membership_orders WHERE (subscription_transaction_id = '" . $subscription_id . "' OR subscription_transaction_id = '"  . esc_sql($subscription_id) . "') AND gateway = 'stripe' ORDER BY timestamp DESC LIMIT 1" );
			}
		}
	...
	function getOrderFromInvoiceEvent($dmrfid_stripe_event)
	{
		//pause here to give DmRFID a chance to finish checkout
		sleep(DMRFID_STRIPE_WEBHOOK_DELAY);

		$invoice_id = $dmrfid_stripe_event->data->object->id;
	...
	function dmrfid_stripeWebhookExit()
	{
		global $logstr;
