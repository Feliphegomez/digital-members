jQuery(document).ready(function() {
	//find status
	var $status = jQuery('#dmrfid_updates_status');
	var $row = 1;
	var $count = 0;
	...
	if($status && $status.length > 0)
	{
		...
		function dmrfid_updates()
		{
			jQuery.ajax({
				...
				success: function(responseHTML){
					...
					{
						alert('Error while running update: ' + responseHTML + ' Try refreshing. If this error occurs again, seek help on the DmRFID member forums.');
					...
					else if(responseHTML.indexOf('[done]') > -1)
					{
						$status.html($status.html() + '\nDone!');
						document.title = '! ' + $title;
						jQuery('#dmrfid_updates_intro').html('All updates are complete.');
						location.reload(1);
					}
					else
					{
						...
						if(progress && progress.length > 0)
							jQuery('#dmrfid_updates_progress').html(progress + ' ' + parseInt(eval(progress[0].replace(/\[|\]/ig, ''))*100) + '%');
						$status.html($status.html() + responseHTML.replace(re, ''));						
						document.title = $cycles[$count%4] + ' ' + $title;
						$update_timer = setTimeout(function() { dmrfid_updates();}, 200);
					}
		...
		var $update_timer = setTimeout(function() { dmrfid_updates();}, 200);
	}
});