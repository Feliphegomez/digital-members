	...
	*/
	...
	if(!empty($_REQUEST['savesettings']))
	{
		...
		*/
		...
		//assume success
		$msg = true;
		$msgt = __("Se actualizó su configuración de pago.", 'digital-members-rfid' );
	}
...
?>
	...
	<form action="" method="post" enctype="multipart/form-data">
        ...
        <h1 class="wp-heading-inline"><?php esc_html_e( 'Pasarela de pago', 'digital-members-rfid' );?> &amp; <?php esc_html_e( 'SSL Settings', 'digital-members-rfid' ); ?></h1>
        <hr class="wp-header-end">

		<p><?php _e('Más información sobre <a title="Miembros digitales RFID - Configuración SSL" target="_blank" href="https://www.managertechnology.com.co/documentation/initial-plugin-setup/ssl/?utm_source=plugin&utm_medium=dmrfid-paymentsettings&utm_campaign=documentation&utm_content=ssl&utm_term=link1">SSL</a> or <a title="Miembros digitales RFID: configuración de la pasarela de pago" target="_blank" href="https://www.managertechnology.com.co/documentation/initial-plugin-setup/step-3-payment-gateway-security/?utm_source=plugin&utm_medium=dmrfid-paymentsettings&utm_campaign=documentation&utm_content=step-3-payment-gateway-security">Configuración de la pasarela de pago</a>.', 'digital-members-rfid' ); ?></p>
		...
		<tbody>
			<tr class="dmrfid_settings_divider">
				<td colspan="2">
					<hr />
					<h3><?php _e('Cambiar pasarela', 'digital-members-rfid' ); ?></h3>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top">
					<label for="gateway"><?php _e('Pasarela de pago', 'digital-members-rfid' );?>:</label>
				</th>
				<td>
			...
			<tr>
				<th scope="row" valign="top">
					<label for="gateway_environment"><?php _e('Entorno de pasarela de pago', 'digital-members-rfid' );?>:</label>
				</th>
				<td>
			...
			<tr class="dmrfid_settings_divider">
				<td colspan="2">
					<hr />
					<h3><?php _e('Configuración de moneda e impuestos', 'digital-members-rfid' ); ?></h3>
				</td>
			</tr>
			<tr class="gateway gateway_ <?php echo esc_attr(dmrfid_getClassesForPaymentSettingsField("currency"));?>" <?php if(!empty($gateway) && $gateway != "paypal" && $gateway != "paypalexpress" && $gateway != "check" && $gateway != "paypalstandard" && $gateway != "braintree" && $gateway != "twocheckout" && $gateway != "cybersource" && $gateway != "payflowpro" && $gateway != "stripe" && $gateway != "authorizenet" && $gateway != "gourl") { ?>style="display: none;"<?php } ?>>
				...
				<td>
					...
					<p class="description"><?php _e( 'No todas las divisas serán compatibles con todas las pasarelas. Consulte con su puerta de enlace.', 'digital-members-rfid' ); ?></p>
				</td>
			</tr>
			<tr class="gateway gateway_ <?php echo esc_attr(dmrfid_getClassesForPaymentSettingsField("accepted_credit_cards"));?>" <?php if(!empty($gateway) && $gateway != "authorizenet" && $gateway != "paypal" && $gateway != "stripe" && $gateway != "payflowpro" && $gateway != "braintree" && $gateway != "twocheckout" && $gateway != "cybersource") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label for="creditcards"><?php _e('Tipos de tarjetas de crédito aceptadas', 'digital-members-rfid' );?></label>
				</th>
				<td>
			...
			<tr class="gateway gateway_ <?php echo esc_attr(dmrfid_getClassesForPaymentSettingsField("tax_rate"));?>" <?php if(!empty($gateway) && $gateway != "stripe" && $gateway != "authorizenet" && $gateway != "paypal" && $gateway != "paypalexpress" && $gateway != "check" && $gateway != "paypalstandard" && $gateway != "payflowpro" && $gateway != "braintree" && $gateway != "twocheckout" && $gateway != "cybersource") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label for="tax"><?php _e('Impuesto de venta', 'digital-members-rfid' );?> (<?php _e('optional', 'digital-members-rfid' );?>)</label>
				</th>
				<td>
					<?php _e('Estado fiscal', 'digital-members-rfid' );?>:
					<input type="text" id="tax_state" name="tax_state" value="<?php echo esc_attr($tax_state)?>" class="small-text" /> (<?php _e('abreviatura, p. ej. "PA"', 'digital-members-rfid' );?>)
					&nbsp; <?php _e('Tasa de impuesto', 'digital-members-rfid' ); ?>:
					<input type="text" id="tax_rate" name="tax_rate" size="10" value="<?php echo esc_attr($tax_rate)?>" class="small-text" /> (<?php _e('decimal, p. ej. "0.06"', 'digital-members-rfid' );?>)
					<p class="description"><?php _e('Solo nosotros. Si se proporcionan valores, se aplicarán impuestos a los miembros que realicen pedidos desde el estado seleccionado. <br /> Para las reglas de impuestos fuera de los EE. UU. O más complejas, use <a target = "_ blank" href = "https: // www. managertechnology.com.co/non-us-taxes-digital-members-rfid/?utm_source=plugin&utm_medium=dmrfid-paymentsettings&utm_campaign=blog&utm_content=non-us-taxes-digital-members-rfid">dmrfid_tax filter </a>.', 'digital-members-rfid' );?></p>
				</td>
			</tr>
			...
			<tr class="dmrfid_settings_divider">
				<td colspan="2">
					<hr />
					<h3><?php _e('Configuración SSL', 'digital-members-rfid' ); ?></h3>
				</td>
			</tr>
			<tr class="gateway gateway_ <?php echo esc_attr(dmrfid_getClassesForPaymentSettingsField("use_ssl"));?>">
				<th scope="row" valign="top">
					<label for="use_ssl"><?php _e('Forzar SSL', 'digital-members-rfid' );?>:</label>
				</th>
				<td>
					<?php
						if( dmrfid_check_site_url_for_https() ) {
							...
							?>
							<p class="description"><?php _e( 'La URL de su sitio comienza con https: //, por lo que DmRFID permitirá que todo su sitio se sirva a través de HTTPS.', 'digital-members-rfid' ); ?></p>
							<?php
						} else {
							...
							<p class="description"><?php _e('Recomendado: si. Pruebe la configuración de redirecciones de JavaScript si tiene problemas con bucles de redireccionamiento infinitos.', 'digital-members-rfid' ); ?></p>
							<?php
						}
					?>
				</td>
			...
			<tr>
				<th scope="row" valign="top">
					<label for="sslseal"><?php _e('Código de sello SSL', 'digital-members-rfid' );?>:</label>
				</th>
				<td>
					<textarea id="sslseal" name="sslseal" rows="3" cols="50" class="large-text"><?php echo stripslashes(esc_textarea($sslseal))?></textarea>
					<p class="description"><?php _e('Su <strong> <a target = "_blank" href="https://www.managertechnology.com.co/documentation/initial-plugin-setup/ssl/?utm_source=plugin&utm_medium=dmrfid-paymentsettings&utm_campaign=documentation&utm_content=ssl=link&utm_ter "> Certificado SSL </a> </strong> debe ser instalado por su proveedor de alojamiento web. Utilice este campo para mostrar su sello u otras imágenes de comerciantes de confianza. Este campo no acepta JavaScript.', 'digital-members-rfid' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row" valign="top">
					<label for="nuclear_HTTPS"><?php _e('Filtro de URL HTTPS adicional', 'digital-members-rfid' );?>:</label>
				</th>
				<td>
