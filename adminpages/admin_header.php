<?php
	...
	if(!$dmrfid_ready)
	{
		...
		if(empty($dmrfid_level_ready) && empty($edit) && $view != "dmrfid-membershiplevels")
			$msgt .= " <a href=\"" . admin_url('admin.php?page=dmrfid-membershiplevels&edit=-1') . "\">" . __("Add a membership level to get started.", 'paid-memberships-pro' ) . "</a>";
		elseif($dmrfid_level_ready && !$dmrfid_pages_ready && $view != "dmrfid-pagesettings")
			$msgt .= " <strong>" . __( 'Next step:', 'paid-memberships-pro' ) . "</strong> <a href=\"" . admin_url('admin.php?page=dmrfid-pagesettings') . "\">" . __("Set up the membership pages", 'paid-memberships-pro' ) . "</a>.";
		elseif($dmrfid_level_ready && $dmrfid_pages_ready && !$dmrfid_gateway_ready && $view != "dmrfid-paymentsettings" && ! dmrfid_onlyFreeLevels())
			$msgt .= " <strong>" . __( 'Next step:', 'paid-memberships-pro' ) . "</strong> <a href=\"" . admin_url('admin.php?page=dmrfid-paymentsettings') . "\">" . __("Set up your SSL certificate and payment gateway", 'paid-memberships-pro' ) . "</a>.";
	...
	if(!dmrfid_checkLevelForStripeCompatibility())
	{
		...
		elseif($view == "dmrfid-membershiplevels")
			...
			$msgt .= " <a href=\"" . admin_url('admin.php?page=dmrfid-membershiplevels') . "\">" . __("Please edit your levels", 'paid-memberships-pro' ) . "</a>.";
	}
	...
	if ( ! dmrfid_check_discount_code_for_gateway_compatibility() ) {
		...
		if ( $view == 'dmrfid-discountcodes' && ! empty($_REQUEST['edit']) && $_REQUEST['edit'] > 0 ) {
			...
			$msgt .= " <a href=\"" . admin_url('admin.php?page=dmrfid-discountcodes') . "\">" . __("Please edit your discount codes", 'paid-memberships-pro' ) . "</a>.";
...
<div class="wrap dmrfid_admin">
	<div class="dmrfid_banner">
		<a class="dmrfid_logo" title="Digital Members RFID - Membership Plugin for WordPress" target="_blank" href="<?php echo dmrfid_https_filter("https://www.paidmembershipspro.com/?utm_source=plugin&utm_medium=dmrfid-admin-header&utm_campaign=homepage")?>"><img src="<?php echo DMRFID_URL?>/images/Paid-Memberships-Pro.png" width="350" height="75" border="0" alt="Digital Members RFID(c) - All Rights Reserved" /></a>
		<div class="dmrfid_meta">
			<span class="dmrfid_version">v<?php echo DMRFID_VERSION?></span>
			<a target="_blank" href="<?php echo dmrfid_https_filter("https://www.paidmembershipspro.com/documentation/?utm_source=plugin&utm_medium=dmrfid-admin-header&utm_campaign=documentation")?>"><?php _e('Documentation', 'paid-memberships-pro' );?></a>
			<a target="_blank" href="https://www.paidmembershipspro.com/pricing/?utm_source=plugin&utm_medium=dmrfid-admin-header&utm_campaign=pricing&utm_content=get-support"><?php _e('Get Support', 'paid-memberships-pro' );?></a>

			<?php if ( dmrfid_license_isValid() ) { ?>
				<?php printf(__( '<a class="dmrfid_license_tag dmrfid_license_tag-valid" href="%s">Valid License</a>', 'paid-memberships-pro' ), admin_url( 'admin.php?page=dmrfid-license' ) ); ?>				
			<?php } elseif ( ! defined( 'DMRFID_LICENSE_NAG' ) || DMRFID_LICENSE_NAG == true ) { ?>
				<?php printf(__( '<a class="dmrfid_license_tag dmrfid_license_tag-invalid" href="%s">No License</a>', 'paid-memberships-pro' ), admin_url( 'admin.php?page=dmrfid-license' ) ); ?>
			<?php } ?>
	...
	<nav class="nav-tab-wrapper">
		<?php if(current_user_can('dmrfid_dashboard')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-dashboard');?>" class="nav-tab<?php if($view == 'dmrfid-dashboard') { ?> nav-tab-active<?php } ?>"><?php _e('Dashboard', 'paid-memberships-pro' );?></a>
		<?php } ?>

		<?php if(current_user_can('dmrfid_memberslist')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-memberslist');?>" class="nav-tab<?php if($view == 'dmrfid-memberslist') { ?> nav-tab-active<?php } ?>"><?php _e('Members', 'paid-memberships-pro' );?></a>
		<?php } ?>

		<?php if(current_user_can('dmrfid_orders')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-orders');?>" class="nav-tab<?php if($view == 'dmrfid-orders') { ?> nav-tab-active<?php } ?>"><?php _e('Orders', 'paid-memberships-pro' );?></a>
		<?php } ?>

		<?php if(current_user_can('dmrfid_reports')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-reports');?>" class="nav-tab<?php if($view == 'dmrfid-reports') { ?> nav-tab-active<?php } ?>"><?php _e('Reports', 'paid-memberships-pro' );?></a>
		<?php } ?>

		<?php if(current_user_can('dmrfid_membershiplevels')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-membershiplevels');?>" class="nav-tab<?php if( in_array( $view, array( 'dmrfid-membershiplevels', 'dmrfid-discountcodes', 'dmrfid-pagesettings', 'dmrfid-paymentsettings', 'dmrfid-emailsettings', 'dmrfid-advancedsettings' ) ) ) { ?> nav-tab-active<?php } ?>"><?php _e('Settings', 'paid-memberships-pro' );?></a>
		<?php } ?>

		<?php if(current_user_can('dmrfid_addons')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-addons');?>" class="nav-tab<?php if($view == 'dmrfid-addons') { ?> nav-tab-active<?php } ?>"><?php _e('Add Ons', 'paid-memberships-pro' );?></a>
		<?php } ?>

		<?php if(current_user_can('manage_options')) { ?>
			<a href="<?php echo admin_url('admin.php?page=dmrfid-license');?>" class="nav-tab<?php if($view == 'dmrfid-license') { ?> nav-tab-active<?php } ?>"><?php _e('License', 'paid-memberships-pro' );?></a>
		<?php } ?>
	</nav>
	...
	<?php if( $view == 'dmrfid-membershiplevels' || $view == 'dmrfid-discountcodes' || $view == 'dmrfid-pagesettings' || $view == 'dmrfid-paymentsettings' || $view == 'dmrfid-emailsettings' || $view == 'dmrfid-advancedsettings' ) { ?>
		<ul class="subsubsub">
			<?php if(current_user_can('dmrfid_membershiplevels')) { ?>
				<li><a href="<?php echo admin_url('admin.php?page=dmrfid-membershiplevels');?>" title="<?php _e('Membership Levels', 'paid-memberships-pro' );?>" class="<?php if($view == 'dmrfid-membershiplevels') { ?>current<?php } ?>"><?php _e('Levels', 'paid-memberships-pro' );?></a>&nbsp;|&nbsp;</li>
			<?php } ?>

			<?php if(current_user_can('dmrfid_discountcodes')) { ?>
				<li><a href="<?php echo admin_url('admin.php?page=dmrfid-discountcodes');?>" title="<?php _e('Discount Codes', 'paid-memberships-pro' );?>" class="<?php if($view == 'dmrfid-discountcodes') { ?>current<?php } ?>"><?php _e('Discount Codes', 'paid-memberships-pro' );?></a>&nbsp;|&nbsp;</li>
			<?php } ?>

			<?php if(current_user_can('dmrfid_pagesettings')) { ?>
				<li><a href="<?php echo admin_url('admin.php?page=dmrfid-pagesettings');?>" title="<?php _e('Page Settings', 'paid-memberships-pro' );?>" class="<?php if($view == 'dmrfid-pagesettings') { ?>current<?php } ?>"><?php _e('Pages', 'paid-memberships-pro' );?></a>&nbsp;|&nbsp;</li>
			<?php } ?>

			<?php if(current_user_can('dmrfid_paymentsettings')) { ?>
				<li><a href="<?php echo admin_url('admin.php?page=dmrfid-paymentsettings');?>" title="<?php _e('Payment Gateway &amp; SSL Settings', 'paid-memberships-pro' );?>" class="<?php if($view == 'dmrfid-paymentsettings') { ?>current<?php } ?>"><?php _e('Payment Gateway &amp; SSL', 'paid-memberships-pro' );?></a>&nbsp;|&nbsp;</li>
			<?php } ?>

			<?php if(current_user_can('dmrfid_emailsettings')) { ?>
				<li><a href="<?php echo admin_url('admin.php?page=dmrfid-emailsettings');?>" title="<?php _e('Email Settings', 'paid-memberships-pro' );?>" class="<?php if($view == 'dmrfid-emailsettings') { ?>current<?php } ?>"><?php _e('Email', 'paid-memberships-pro' );?></a>&nbsp;|&nbsp;</li>
			<?php } ?>

			<?php if(current_user_can('dmrfid_advancedsettings')) { ?>
				<li><a href="<?php echo admin_url('admin.php?page=dmrfid-advancedsettings');?>" title="<?php _e('Advanced Settings', 'paid-memberships-pro' );?>" class="<?php if($view == 'dmrfid-advancedsettings') { ?>current<?php } ?>"><?php _e('Advanced', 'paid-memberships-pro' );?></a></li>
			<?php } ?>
		</ul>
		<br class="clear" />
	<?php } ?>
