...
function dmrfid_getLevelCost( &$level, $tags = true, $short = false ) {
	// initial payment
	if ( ! $short ) {
		$r = sprintf( __( 'The price for membership is <strong>%s</strong> now', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->initial_payment ) );
	} else {
		if ( dmrfid_isLevelFree( $level ) ) {
			$r = '<strong>' . __('Free', 'paid-memberships-pro' ) . '</strong>';
		} else {
			$r = sprintf( __( '<strong>%s</strong> now', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->initial_payment ) );
		}
	}
	...
	if ( $level->billing_amount != '0.00' ) {
		if ( $level->billing_limit > 1 ) {
			if ( $level->cycle_number == '1' ) {
				$r .= sprintf( __( ' and then <strong>%1$s per %2$s for %3$d more %4$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->billing_amount ), dmrfid_translate_billing_period( $level->cycle_period ), $level->billing_limit, dmrfid_translate_billing_period( $level->cycle_period, $level->billing_limit ) );
			} else {
				$r .= sprintf( __( ' and then <strong>%1$s every %2$d %3$s for %4$d more payments</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->billing_amount ), $level->cycle_number, dmrfid_translate_billing_period( $level->cycle_period, $level->cycle_number ), $level->billing_limit );
			}
		} elseif ( $level->billing_limit == 1 ) {
			$r .= sprintf( __( ' and then <strong>%1$s after %2$d %3$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->billing_amount ), $level->cycle_number, dmrfid_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
		} else {
			if ( $level->billing_amount === $level->initial_payment ) {
				if ( $level->cycle_number == '1' ) {
					if ( ! $short ) {
						$r = sprintf( __( 'The price for membership is <strong>%1$s per %2$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->initial_payment ), dmrfid_translate_billing_period( $level->cycle_period ) );
					} else {
						$r = sprintf( __( '<strong>%1$s per %2$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->initial_payment ), dmrfid_translate_billing_period( $level->cycle_period ) );
					}
				} else {
					if ( ! $short ) {
						$r = sprintf( __( 'The price for membership is <strong>%1$s every %2$d %3$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->initial_payment ), $level->cycle_number, dmrfid_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
					} else {
						$r = sprintf( __( '<strong>%1$s every %2$d %3$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->initial_payment ), $level->cycle_number, dmrfid_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
					}
				}
			} else {
				if ( $level->cycle_number == '1' ) {
					$r .= sprintf( __( ' and then <strong>%1$s per %2$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->billing_amount ), dmrfid_translate_billing_period( $level->cycle_period ) );
				} else {
					$r .= sprintf( __( ' and then <strong>%1$s every %2$d %3$s</strong>.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->billing_amount ), $level->cycle_number, dmrfid_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
				}
			}
		}
	} else {
	...
	if ( $level->trial_limit ) {
		if ( $level->trial_amount == '0.00' ) {
			...
			if ( $level->trial_limit == '1' ) {
				$r .= ' ' . sprintf( __( 'After your initial payment, your first payment will cost %s.', 'paid-memberships-pro' ), dmrfid_formatPrice( $level->trial_amount ) );
			} else {
				$r .= ' ' . sprintf( __( 'After your initial payment, your first %1$d payments will cost %2$s.', 'paid-memberships-pro' ), $level->trial_limit, dmrfid_formatPrice( $level->trial_amount ) );
			}
		}
	}
...
function dmrfid_getLevelsCost( &$levels, $tags = true, $short = false ) {
	...
	// initial payment
	if ( ! $short ) {
		$r = sprintf( __( 'The price for membership is <strong>%s</strong> now', 'paid-memberships-pro' ), dmrfid_formatPrice( $initpmt ) );
	} else {
		$r = sprintf( __( '<strong>%s</strong> now', 'paid-memberships-pro' ), dmrfid_formatPrice( $initpmt ) );
	}
		...
		foreach ( $recurpmts as $curperiod => $curpddata ) {
			foreach ( $curpddata as $curcyclenum => $curcycledata ) {
				foreach ( $curcycledata as $curbilllimit => $curtotal ) {
					if ( $curbilllimit > 1 ) {
						if ( $curcyclenum == '1' ) {
							$billtextparts[] = sprintf( __( '<strong>%1$s per %2$s for %3$d more %4$s</strong>', 'paid-memberships-pro' ), dmrfid_formatPrice( $curtotal ), dmrfid_translate_billing_period( $curperiod ), $curbilllimit, dmrfid_translate_billing_period( $curperiod, $curbilllimit ) );
						} else {
							$billtextparts[] = sprintf( __( '<strong>%1$s every %2$d %3$s for %4$d more payments</strong>', 'paid-memberships-pro' ), dmrfid_formatPrice( $curtotal ), $curcyclenum, dmrfid_translate_billing_period( $curperiod, $curcyclenum ), $curbilllimit );
						}
					} elseif ( $curbilllimit == 1 ) {
						$billtextparts[] = sprintf( __( '<strong>%1$s after %2$d %3$s</strong>', 'paid-memberships-pro' ), dmrfid_formatPrice( $curtotal ), $curcyclenum, dmrfid_translate_billing_period( $curperiod, $curcyclenum ) );
					} else {
						if ( $curcyclenum == '1' ) {
							$billtextparts[] = sprintf( __( '<strong>%1$s every %2$s</strong>', 'paid-memberships-pro' ), dmrfid_formatPrice( $curtotal ), dmrfid_translate_billing_period( $curperiod ) );
						} else {
							$billtextparts[] = sprintf( __( '<strong>%1$s every %2$d %3$s</strong>', 'paid-memberships-pro' ), dmrfid_formatPrice( $curtotal ), $curcyclenum, dmrfid_translate_billing_period( $curperiod, $curcyclenum ) );
						}
					}
				}
			}
...
function dmrfid_getLevelExpiration( &$level ) {
	if ( $level->expiration_number ) {
		$expiration_text = sprintf( __( 'Membership expires after %1$d %2$s.', 'paid-memberships-pro' ), $level->expiration_number, dmrfid_translate_billing_period( $level->expiration_period, $level->expiration_number ) );
	} else {
		$expiration_text = '';
	}
...
function dmrfid_getLevelsExpiration( &$levels ) {
	...
	foreach ( $levels as $curlevel ) {
		if ( $curlevel->expiration_number ) {
			$expirystrings[] = sprintf( __( '%1$s membership expires after %2$d %3$s', 'paid-memberships-pro' ), $curlevel->name, $curlevel->expiration_number, dmrfid_translate_billing_period( $curlevel->expiration_period, $curlevel->expiration_number ) );
		} else {
			$ongoinglevelnum++;
		}
	}
...
function dmrfid_changeMembershipLevel( $level, $user_id = null, $old_level_status = 'inactive', $cancel_level = null ) {
	...
	if ( empty( $user_id ) ) {
		$dmrfid_error = __( 'User ID not found.', 'paid-memberships-pro' );
		return false;
	}
	...
	if ( empty( $level ) ) {
		...
		if ( empty( $level['membership_id'] ) ) {
			$dmrfid_error = __( 'No membership_id specified in dmrfid_changeMembershipLevel.', 'paid-memberships-pro' );
			return false;
		}

		$level_obj = dmrfid_getLevel( $level['membership_id'] );
		if ( empty( $level_obj ) ) {
			$dmrfid_error = __( 'Invalid level.', 'paid-memberships-pro' );
			return false;
		}
		unset( $level_obj );
	} else {
	...
	if ( ! is_array( $level ) ) {
		...
		if ( dmrfid_hasMembershipLevel( $level, $user_id ) ) {
			$dmrfid_error = __( 'not changing?', 'paid-memberships-pro' );
	...
	if ( $old_levels && $dmrfid_deactivate_old_levels ) {
		foreach ( $old_levels as $old_level ) {
			...
			if ( ! $wpdb->query( $sql ) ) {
				$dmrfid_error = __( 'Error interacting with database', 'paid-memberships-pro' ) . ': ' . ( $wpdb->last_error ? $wpdb->last_error : 'unavailable' );
	...
	if ( ! empty( $level ) ) {
		...
		if ( false === $wpdb->query( $sql ) ) {
			$dmrfid_error = sprintf( __( 'Error interacting with database: %s', 'paid-memberships-pro' ), ( ! empty( $wpdb->last_error ) ? $wpdb->last_error : 'unavailable' ) );
			return false;
		}
	}
