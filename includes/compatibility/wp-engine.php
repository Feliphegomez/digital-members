 ...
 *
 * In addition to this code to fix the DmRFID password reset form, note that
 ...
 */
function dmrfid_wpe_fix_lostpass( $url ) {
	if ( strpos( $url, 'wpe-login=true' ) === false ) {
		$url = add_query_arg( 'wpe-login', true, $url );
	}
	return $url;
}
add_filter( 'lostpassword_url', 'dmrfid_wpe_fix_lostpass' );
 ...
 */
function dmrfid_wpe_fix_resetpass( $url, $path ) {
	if ( strpos( $path, 'action=resetpass' ) !== false && strpos( $path, 'wpe-login=true' ) === false ) {
		$url = add_query_arg( 'wpe-login', 'true', $url );
	}
	return $url;
}
add_filter( 'site_url', 'dmrfid_wpe_fix_resetpass', 10, 2 );
