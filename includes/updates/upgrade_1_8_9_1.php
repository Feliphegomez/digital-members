...
*/
function dmrfid_upgrade_1_8_9_1() {
	global $wpdb;	
	
	//Fixing Stripe orders where user_id/membership_id = 0. (Sets up update via AJAX)
	$orders = $wpdb->get_col("SELECT id FROM $wpdb->dmrfid_membership_orders WHERE gateway = 'stripe' AND user_id = 0 AND membership_id = 0 AND status <> 'error' ");			
	if(!empty($orders))
		dmrfid_addUpdate('dmrfid_upgrade_1_8_9_1_ajax');

	dmrfid_setOption("db_version", "1.891");
	return 1.891;
}
...
*/
function dmrfid_upgrade_1_8_9_1_ajax() {
	global $wpdb;
	...
	$last_order_id = get_option('dmrfid_upgrade_1_8_9_1_last_order_id', 0);
	
	//Fixing old $0 Stripe orders.	
	$orders = $wpdb->get_col("SELECT id FROM $wpdb->dmrfid_membership_orders WHERE id > $last_order_id AND gateway = 'stripe' AND user_id = 0 AND membership_id = 0 AND status <> 'error' ORDER BY id LIMIT 2");
	
	//track progress
	$first_load = get_transient('dmrfid_updates_first_load');
	if($first_load) {
		$total_orders = $wpdb->get_var("SELECT COUNT(id) FROM $wpdb->dmrfid_membership_orders WHERE id > $last_order_id AND gateway = 'stripe' AND user_id = 0 AND membership_id = 0 AND status <> 'error' ");
		update_option('dmrfid_upgrade_1_8_9_1_total', $total_orders, 'no');
		$progress = 0;
	} else {
		$total_orders = get_option('dmrfid_upgrade_1_8_9_1_total', 0);
		$progress = get_option('dmrfid_upgrade_1_8_9_1_progress', 0);
	}
	update_option('dmrfid_upgrade_1_8_9_1_progress', $progress + count($orders), 'no');
	...
	if(empty($orders)) {
		...
		dmrfid_removeUpdate('dmrfid_upgrade_1_8_9_1_ajax');
		delete_option('dmrfid_upgrade_1_8_9_1_last_order_id');
		delete_option('dmrfid_upgrade_1_8_9_1_total');
		delete_option('dmrfid_upgrade_1_8_9_1_progress');
	} else {
		...
		foreach($orders as $order_id) {				
			...
			$subscription = json_decode(wp_remote_retrieve_body(wp_remote_get('https://api.stripe.com/v1/subscriptions/' . $order->subscription_transaction_id, array(
					...
					'sslverify' => FALSE,
					'httpversion' => '1.1',
					'headers'=>array('Authorization' => 'Bearer ' . dmrfid_getOption("stripe_secretkey")),
			    ))));
			...
			if(empty($subscription) || empty($subscription->customer)) {
				...
				if($run)
					$wpdb->query("UPDATE $wpdb->dmrfid_membership_orders SET `status` = 'error', notes = CONCAT(notes, '\nRecurring order we couldn\'t find the subscription.') WHERE id = $order->id LIMIT 1");
				...
				foreach($invoices->data as $invoice) {
					...
					if($invoice->subscription == $order->subscription_transaction_id) {
						...
						if(!empty($old_order)) {
							...
							if($run) {
								$sqlQuery = "UPDATE $wpdb->dmrfid_membership_orders SET user_id = " . $old_order->user_id . ", membership_id = " . $old_order->membership_id . " WHERE user_id = 0 AND membership_id = 0 AND subscription_transaction_id = '" . $order->subscription_transaction_id . "' ";							
								$wpdb->query($sqlQuery);
							}
		...
		update_option('dmrfid_upgrade_1_8_9_1_last_order_id', $last_order_id, 'no');
	}	
}
