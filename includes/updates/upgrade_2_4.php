...
function dmrfid_upgrade_2_4() {
	...
	if(!empty($orders)) {
		if(count($orders) > 10) {
			...
			dmrfid_addUpdate( 'dmrfid_upgrade_2_4_ajax' );
		} else {
            ...
            foreach($orders as $order) {                
                ...
                if ( ! empty( $subscription ) ) {
                    $sqlQuery = "UPDATE $wpdb->dmrfid_membership_orders SET subscription_transaction_id = '" . esc_sql( $subscription->id ) . "' WHERE id = '" . esc_sql( $order->id ) . "' LIMIT 1";
                    $wpdb->query( $sqlQuery );
                }
			}
		}
	...
	dmrfid_setOption( 'db_version', '2.4' );
	return 2.4;
}
...
*/
function dmrfid_upgrade_2_4_ajax() {
	global $wpdb;

	//keeping track of which order we're working on
	$last_order_id = get_option( 'dmrfid_upgrade_2_4_last_order_id', 0 );
	...
	if(empty($orders)) {
		...
		dmrfid_removeUpdate('dmrfid_upgrade_2_4_ajax');
		delete_option( 'dmrfid_upgrade_2_4_last_order_id' );
	} else {
		//less than 10, let's just do them now
		$stripe = new DmRFIDGateway_stripe();
		require_once( ABSPATH . "/wp-includes/pluggable.php" );
		...
		update_option( 'dmrfid_upgrade_2_4_last_order_id', $last_order_id );
	}
}
