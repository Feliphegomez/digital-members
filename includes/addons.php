 ...
 * @since 1.8.5
 */
function dmrfid_setupAddonUpdateInfo() {
	add_filter( 'plugins_api', 'dmrfid_plugins_api', 10, 3 );
	add_filter( 'pre_set_site_transient_update_plugins', 'dmrfid_update_plugins_filter' );
	add_filter( 'http_request_args', 'dmrfid_http_request_args_for_addons', 10, 2 );
	add_action( 'update_option_dmrfid_license_key', 'dmrfid_reset_update_plugins_cache', 10, 2 );
}
add_action( 'init', 'dmrfid_setupAddonUpdateInfo' );

/**
 * Get addon information from DmRFID server.
 *
 * @since  1.8.5
 */
function dmrfid_getAddons() {
	// check if forcing a pull from the server
	$addons = get_option( 'dmrfid_addons', array() );
	$addons_timestamp = get_option( 'dmrfid_addons_timestamp', 0 );
	...
	if ( empty( $addons ) || ! empty( $_REQUEST['force-check'] ) || current_time( 'timestamp' ) > $addons_timestamp + 86400 ) {
		 ...
		 */
		$timeout = apply_filters( 'dmrfid_get_addons_timeout', 5 );
		...
		if ( is_wp_error( $remote_addons ) ) {
			// error
			dmrfid_setMessage( 'Could not connect to the DmRFID License Server to update addon information. Try again later.', 'error' );
			...
			delete_option( 'dmrfid_addons' );
			add_option( 'dmrfid_addons', $addons, null, 'no' );
		}

		// save timestamp of last update
		delete_option( 'dmrfid_addons_timestamp' );
		add_option( 'dmrfid_addons_timestamp', current_time( 'timestamp' ), null, 'no' );
	}
 ...
 */
function dmrfid_getAddonBySlug( $slug ) {
 ...
 */
function dmrfid_update_plugins_filter( $value ) {
	...
	foreach ( $addons as $addon ) {
		...
		if ( ! empty( $addon['License'] ) && version_compare( $plugin_data['Version'], $addon['Version'], '<' ) ) {
			$value->response[ $plugin_file ] = dmrfid_getPluginAPIObjectFromAddon( $addon );
			$value->response[ $plugin_file ]->new_version = $addon['Version'];
		} else {
			$value->no_update[ $plugin_file ] = dmrfid_getPluginAPIObjectFromAddon( $addon );
		}
	}
 ...
 */
function dmrfid_http_request_args_for_addons( $args, $url ) {
 ...
 * @since  1.8.5
 */
function dmrfid_plugins_api( $api, $action = '', $args = null ) {
	...
	// get addon information
	$addon = dmrfid_getAddonBySlug( $args->slug );
	...
	$api = dmrfid_getPluginAPIObjectFromAddon( $addon );
	return $api;
}

/**
 * Convert the format from the dmrfid_getAddons function to that needed for plugins_api
 *
 * @since  1.8.5
 */
function dmrfid_getPluginAPIObjectFromAddon( $addon ) {
	$api                        = new stdClass();
...
/**
 * Force update of plugin update data when the DmRFID License key is updated
 *
 * @since 1.8
 *
 * @param array  $args  Array of request args.
 ...
 * @return array $args Amended array of request args.
 */
function dmrfid_reset_update_plugins_cache( $old_value, $value ) {
	delete_option( 'dmrfid_addons_timestamp' );
	delete_site_transient( 'update_themes' );
}

/**
 * Detect when trying to update a DmRFID Plus plugin without a valid license key.
 *
 * @since 1.9
 */
function dmrfid_admin_init_updating_plugins() {
	...
	/*
        TODO:
		* Check for DmRFID Plug plugins
		* If a plus plugin is found, check the DmRFID license key
...
add_action( 'admin_init', 'dmrfid_admin_init_updating_plugins' );
