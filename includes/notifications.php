 ...
 */
function dmrfid_notifications() {
	if ( current_user_can( 'manage_options' ) ) {		
		$notification = dmrfid_get_next_notification();		
		if ( empty( $notification ) ) {
			exit;
		}
		
		$paused = dmrfid_notifications_pause();		
		if ( $paused && empty( $_REQUEST['dmrfid_notification'] ) && $notification->priority !== 1 ) {
			exit;
		}
		
		// Okay show the notification.
		?>
		<div class="dmrfid_notification" id="<?php echo $notification->id; ?>">
		<?php if ( $notification->dismissable ) { ?>
			<button type="button" class="dmrfid-notice-button notice-dismiss" value="<?php echo $notification->id; ?>"><span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'digital-members-rfid' ); ?></span></button>
		<?php } ?>
			<div class="dmrfid_notification-<?php echo $notification->type; ?>">
...
add_action( 'wp_ajax_dmrfid_notifications', 'dmrfid_notifications' );

/**
 * Get the highest priority applicable notification from the list.
 */
function dmrfid_get_next_notification() {
	global $current_user;	
	...
	if ( ! empty( $_REQUEST['dmrfid_notification'] ) ) {
		delete_transient( 'dmrfid_notifications_' . DMRFID_VERSION );
		$dmrfid_notifications = dmrfid_get_all_notifications();
				
		if ( !empty( $dmrfid_notifications ) ) {
			foreach( $dmrfid_notifications as $notification ) {
				if ( $notification->id == $_REQUEST['dmrfid_notification'] ) {
					return $notification;
				}
			}
	...
	$dmrfid_filtered_notifications = array();
	$archived_notifications = get_user_meta( $current_user->ID, 'dmrfid_archived_notifications', true );
	foreach ( $dmrfid_notifications as $notification ) {		
		if ( ( is_array( $archived_notifications ) && array_key_exists( $notification->id, $archived_notifications ) ) ) {
			continue;
		}

		$dmrfid_filtered_notifications[] = $notification;		
	}	
		
	// Return the first one.
	if ( ! empty( $dmrfid_filtered_notifications ) ) {
		$next_notification = $dmrfid_filtered_notifications[0];
	} else {
 ...
 */
function dmrfid_get_all_notifications() {
	$dmrfid_notifications = get_transient( 'dmrfid_notifications_' . DMRFID_VERSION );
	...
	if ( empty( $dmrfid_notifications ) ) {
		...
		$dmrfid_notifications = json_decode( wp_remote_retrieve_body( $remote_notifications ) );
		
		// Update transient if we got something.
		if ( ! empty( $dmrfid_notifications ) ) {
			set_transient( 'dmrfid_notifications_' . DMRFID_VERSION, $dmrfid_notifications, 86400 );
		}
	}
	
	// We expect an array.
	if( ! is_array( $dmrfid_notifications ) ) {
		$dmrfid_notifications = array();
	}
	
	// Filter notifications by start/end date.
	$dmrfid_active_notifications = array();
	foreach( $dmrfid_notifications as $notification ) {		
		$dmrfid_active_notifications[] = $notification;
	}
		
	// Filter out notifications based on show/hide rules.
	$dmrfid_applicable_notifications = array();
	foreach( $dmrfid_active_notifications as $notification ) {
		if ( dmrfid_is_notification_applicable( $notification ) ) {
			$dmrfid_applicable_notifications[] = $notification;			
		}
	}
		
	// Sort by priority.	
	$dmrfid_applicable_notifications = wp_list_sort( $dmrfid_applicable_notifications, 'priority' );
	
	return $dmrfid_applicable_notifications;
}
 ...
 */
function dmrfid_is_notification_applicable( $notification ) {
	// If one is specified by URL parameter, it's allowed.
	if ( !empty( $_REQUEST['dmrfid_notification'] ) && $notification->id == intval( $_REQUEST['dmrfid_notification'] ) ) {
		return true;
	}
	...
	if ( $notification->priority > dmrfid_get_max_notification_priority() ) {
		return false;
	}
	
	// Check show rules.
	if ( ! dmrfid_should_show_notification( $notification ) ) {
		return false;
	}
	
	// Check hide rules.
	if ( dmrfid_should_hide_notification( $notification ) ) {
		return false;
	}
 ...
 * @param object $notification The notification object.
 */
function dmrfid_should_show_notification( $notification ) {
	// default to showing
	$show = true;
	
	if ( !empty( $notification->show_if ) ) {
		foreach( $notification->show_if as $test => $data ) {
			$test_function = 'dmrfid_notification_test_' . $test;
 ...
 * @param object $notification The notification object.
 */
function dmrfid_should_hide_notification( $notification ) {		
 ...
 */
function dmrfid_notification_test_plugins_active( $plugins ) {
	if ( ! is_array( $plugins ) ) {
		$plugins = array( $plugins );
	}

	foreach( $plugins as $plugin ) {
		if ( ! dmrfid_is_plugin_active( $plugin ) ) {			
			return false;
		}
	}
 ...
 */
function dmrfid_notification_test_check_plugin_version( $data ) {
	if ( ! is_array( $data ) ) {
		return false;
	}
	...
	return dmrfid_check_plugin_version( $data[0], $data[1], $data[2] );
}

/**
 * DmRFID license type test.
 * @param string $license DmRFID license type to check for.
 * @returns bool true if the DmRFID license type matches.
 */
function dmrfid_notification_test_dmrfid_license( $license_type ) {
	if ( empty( $license_type ) ) {
		// If no license type, check they DON'T have a valid license key
		$valid = ! dmrfid_license_isValid();
	} else {
		// Check if they have a valid key of the type specified
		$valid = dmrfid_license_isValid( NULL, $license_type );
	}
 ...
 */
function dmrfid_notification_test_dmrfid_num_members( $data ) {
	global $wpdb;
	static $num_members;
	...
	if ( ! isset( $num_members ) ) {
		$sqlQuery = "SELECT COUNT(*) FROM ( SELECT user_id FROM $wpdb->dmrfid_memberships_users WHERE status = 'active' GROUP BY user_id ) t1";
		$num_members = $wpdb->get_var( $sqlQuery );
	}

	return dmrfid_int_compare( $num_members, $data[1], $data[0] );
}

/**
 * DmRFID number of levels test.
 ...
 */
function dmrfid_notification_test_dmrfid_num_levels( $data ) {
	global $wpdb;
	static $num_levels;
	...
	if ( ! isset( $num_levels ) ) {
		$sqlQuery = "SELECT COUNT(*) FROM $wpdb->dmrfid_membership_levels";
		$num_levels = $wpdb->get_var( $sqlQuery );
	}

	return dmrfid_int_compare( $num_levels, $data[1], $data[0] );
}

/**
 * DmRFID number of discount codes test.
 ...
 */
function dmrfid_notification_test_dmrfid_num_discount_codes( $data ) {
	global $wpdb;
	static $num_codes;
	...
	if ( ! isset( $num_codes ) ) {
		$sqlQuery = "SELECT COUNT(*) FROM $wpdb->dmrfid_discount_codes";
		$num_codes = $wpdb->get_var( $sqlQuery );
	}

	return dmrfid_int_compare( $num_codes, $data[1], $data[0] );
}

/**
 * DmRFID number of orders test.
 ...
 */
function dmrfid_notification_test_dmrfid_num_orders( $data ) {
	global $wpdb;
	static $num_orders;
	...
	if ( ! isset( $num_orders ) ) {
		$sqlQuery = "SELECT COUNT(*) FROM $wpdb->dmrfid_membership_orders WHERE gateway_environment = 'live' AND status NOT IN('refunded', 'review', 'token', 'error')";
		$num_orders = $wpdb->get_var( $sqlQuery );
	}

	return dmrfid_int_compare( $num_orders, $data[1], $data[0] );
}
 ...
 */
function dmrfid_notification_test_dmrfid_revenue( $data ) {
	global $wpdb;
	static $revenue;
	...
	if ( ! isset( $revenue ) ) {
		$sqlQuery = "SELECT SUM(total) FROM $wpdb->dmrfid_membership_orders WHERE gateway_environment = 'live' AND status NOT IN('refunded', 'review', 'token', 'error')";
		$revenue = $wpdb->get_var( $sqlQuery );
	}

	return dmrfid_int_compare( $revenue, $data[1], $data[0] );
}
 ...
 */
function dmrfid_notification_test_dmrfid_setting( $data ) {
	if ( ! is_array( $data ) || !isset( $data[0] ) || !isset( $data[1] ) ) {
		return false;
	}
	
	// remove the dmrfid_ prefix if given
	if ( strpos( $data[0], 'dmrfid_' ) === 0 ) {
		$data[0] = substr( $data[0], 6, strlen( $data[0] ) - 6 );
	}
		
	$option_value = dmrfid_getOption( $data[0] );	
...
function dmrfid_get_max_notification_priority() {
	...
	if ( ! isset( $max_priority ) ) {
		...
		$max_priority = apply_filters( 'dmrfid_max_notification_priority', $max_priority );
	}
 ...
 */
function dmrfid_notifications_pause() {
	global $current_user;
...
function dmrfid_hide_notice() {
	...
	update_user_meta( $current_user->ID, 'dmrfid_archived_notifications', $archived_notifications );
	exit;
}
add_action( 'wp_ajax_dmrfid_hide_notice', 'dmrfid_hide_notice' );

/**
 * Show Powered by Digital Members RFID comment (only visible in source) in the footer.
 */
function dmrfid_link() { ?>
Memberships powered by Digital Members RFID v<?php echo DMRFID_VERSION; ?>.
<?php }
function dmrfid_footer_link() {
	if ( ! dmrfid_getOption( 'hide_footer_link' ) ) { ?>
		<!-- <?php echo dmrfid_link()?> -->
	<?php }
}
add_action( 'wp_footer', 'dmrfid_footer_link' );