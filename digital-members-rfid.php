...
require_once( DMRFID_DIR . '/classes/class-deny-network-activation.php' );   // stop PMPro from being network activated
...
require_once( DMRFID_DIR . '/classes/class.pmproemail.php' );        // setup and filter emails sent by PMPro
require_once( DMRFID_DIR . '/classes/class-pmpro-levels.php' );
require_once( DMRFID_DIR . '/classes/class-pmpro-admin-activity-email.php' );        // setup the admin activity email
...
require_once( DMRFID_DIR . '/classes/class-pmpro-members-list-table.php' ); // Members List
...
require_once( DMRFID_DIR . '/includes/menus.php' );          		// custom menu functions for PMPro
require_once( DMRFID_DIR . '/includes/notifications.php' );          // check for notifications at PMPro, shown in PMPro settings
...
require_once( DMRFID_DIR . '/includes/capabilities.php' );           // manage PMPro capabilities for roles
...
require_once( DMRFID_DIR . '/includes/widgets.php' );          		// widgets for PMPro

require_once( DMRFID_DIR . '/shortcodes/checkout_button.php' );      // [pmpro_checkout_button] shortcode to show link to checkout for a level
require_once( DMRFID_DIR . '/shortcodes/membership.php' );           // [membership] shortcode to hide/show member content
require_once( DMRFID_DIR . '/shortcodes/pmpro_account.php' );        // [pmpro_account] shortcode to show account information
require_once( DMRFID_DIR . '/shortcodes/pmpro_login.php' );      // [pmpro_login] shortcode to show a login form or logged in member info and menu.
require_once( DMRFID_DIR . '/shortcodes/pmpro_member.php' );         // [pmpro_member] shortcode to show user fields
require_once( DMRFID_DIR . '/shortcodes/pmpro_member_profile_edit.php' );         // [pmpro_member_profile_edit] shortcode to allow members to edit their profile

// load gateway
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway.php' ); // loaded by memberorder class when needed

// load payment gateway class
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_authorizenet.php' );

if ( version_compare( PHP_VERSION, '5.4.45', '>=' ) ) {
	require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_braintree.php' );
}

require_once( DMRFID_DIR . '/classes/class-pmpro-discount-codes.php' ); // loaded by memberorder class when needed

require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_check.php' );
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_cybersource.php' );
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_payflowpro.php' );
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_paypal.php' );
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_paypalexpress.php' );
require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_paypalstandard.php' );

if ( version_compare( PHP_VERSION, '5.3.29', '>=' ) ) {
	require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_stripe.php' );
	require_once( DMRFID_DIR . '/includes/lib/stripe-apple-pay/stripe-apple-pay.php' ); // rewrite rules to set up Apple Pay.
}

require_once( DMRFID_DIR . '/classes/gateways/class.pmprogateway_twocheckout.php' );
...
define( 'DMRFID_DOMAIN', pmpro_getDomainFromURL( site_url() ) );
